...
#import numpy as np
from laws import __acknowledgement__, __author__
from . import globals
from laws.management_modules.checks import *
from laws.management_modules.timestep import *
from laws.management_modules.replace_pcr import *
from laws.management_modules.messages import *

import difflib  # to check the closest word in settingsfile, if an error occurs
import math
from laws.management_modules.dynamicModel import *
...
def get_latitudes_1d():
    """
    Return cell-centre latitudes for the active model domain as a 1D array.

    The array length equals the number of unmasked cells. Its values follow
    the same compressed-cell ordering used by ``compressArray`` and
    ``writenetcdf``.

    :return: One-dimensional NumPy array of cell-centre latitudes (float64).
    """

    # Build a 2D latitude field from the configured mask and grid attributes.
    rows, cols = maskinfo['shape']
    cell = maskmapAttr['cell']
    yu = maskmapAttr['y']
    yd = yu - rows * cell

    # Cell-centre latitudes, consistent with writenetcdf dimension generation.
    lats_row = np.linspace(yu, yd, rows, endpoint=False) - cell / 2.0
    lat2d = np.repeat(lats_row[:, None], cols, axis=1)

    # Compress with the 2D mask to preserve the standard model-cell ordering.
    latC = np.ma.compressed(np.ma.masked_array(lat2d, maskinfo['mask']))
    return latC

def lonlat_to_rowcol(lon, lat):
    """
    Return the row and column indices for a longitude-latitude coordinate.

    The coordinate is mapped to the current mask grid using integer indices:
    col = int((lon - x_left) * invcell)
    row = int((y_up - lat) * invcell)

    :return: ``(row, col)``, or ``None`` if the coordinate is outside the grid.
    """
    x_left = maskmapAttr['x']
    y_up = maskmapAttr['y']
    invcell = maskmapAttr['invcell']
    rows, cols = maskinfo['shape']
...
def get_compressed_index_from_lonlat(lon, lat):
    """
    Return the compressed 1D array index for a longitude-latitude coordinate.

    Return ``None`` when the coordinate is outside the grid or its cell is
    masked. The index follows the same compressed-cell ordering used by
    ``compressArray`` and ``writenetcdf``.
    """
    rc = lonlat_to_rowcol(lon, lat)
    if rc is None:
        return None
    row, col = rc
    rows, cols = maskinfo['shape']
    ...
    # A masked cell has no corresponding compressed index.
    if maskinfo['maskflat'][flat_idx]:
        return None

    # Locate flat_idx within the ordered set of unmasked cell positions.
    valid_positions = np.flatnonzero(~maskinfo['maskflat'])
    pos = np.searchsorted(valid_positions, flat_idx)
    if pos < valid_positions.size and valid_positions[pos] == flat_idx:
        return int(pos)
    return None

def calculate_coldest_day_indices_2d_to_1d():
    """
    Calculate the multi-year monthly mean air temperature from NetCDF data.

    For each grid cell, return the day-of-year index for the 15th day of its
    coldest month. The indices are calculated as a 2D array and then compressed
    to 1D using the standard CWatM cell ordering. Chunked reads and vectorized
    operations are used to reduce memory use and improve performance.

    :return: Compressed 1D array containing the coldest-month day-of-year index.
    """

    temp_maps_name = 'TavgMaps'
    temp_files = meteofiles[temp_maps_name]

    if not temp_files:
        raise ValueError(f"No temperature data files found for: {temp_maps_name}")

    file_info = list(temp_files.values())[0]
    filename = file_info[0]
    start_idx = file_info[1]
    end_idx = file_info[2]
    cutcheck = file_info[6]
    turn_latitude = file_info[8]
    buffer = file_info[9]
    shapey = file_info[10]
    shapex = file_info[11]
    value = file_info[5]

    print(f"Processing temperature file: {filename}")

    try:
        nf1 = Dataset(filename, 'r')
        nctime = nf1.variables['time']

        start_date = num2date(nctime[start_idx], units=nctime.units, calendar=nctime.calendar)
        end_date = num2date(nctime[end_idx], units=nctime.units, calendar=nctime.calendar)

        start_year = start_date.year
        end_year = end_date.year

        print(f"Automatically detected year range: {start_year}-{end_year}")
        ...
        if cutcheck:
            ...
            loc = [0, shapey, 0, shapex]

        time_axis = nctime[start_idx:end_idx+1]
        dates = num2date(time_axis, units=nctime.units, calendar=nctime.calendar)
        ...
        if not target_dates:
            raise ValueError(
                f"No valid temperature data found between {start_year} and {end_year}"
            )

        print(f"Found {len(target_dates)} daily records to process")

        chunk_size = min(365, len(target_indices))
        monthly_temps = {}
        monthly_counts = {}

        first_chunk_size = min(chunk_size, len(target_indices))
        first_chunk_indices = target_indices[:first_chunk_size]

        data_chunk = nf1.variables[value][start_idx + first_chunk_indices[0]:start_idx + first_chunk_indices[-1] + 1,
                                         loc[0]:loc[1], loc[2]:loc[3]]

        if turn_latitude:
            data_chunk = np.flip(data_chunk, axis=1)

        data_chunk = data_chunk.astype(np.float32)
        data_chunk[data_chunk > 1e15] = np.nan

        for month in range(1, 13):
            monthly_temps[month] = np.zeros_like(data_chunk[0, :, :], dtype=np.float32)
            monthly_counts[month] = 0

        for i, chunk_idx in enumerate(first_chunk_indices):
            date_idx = target_indices.index(chunk_idx)
            current_date = target_dates[date_idx]
            month_idx = current_date.month

            monthly_temps[month_idx] += data_chunk[i, :, :]
            monthly_counts[month_idx] += 1

        for chunk_start in range(first_chunk_size, len(target_indices), chunk_size):
            chunk_end = min(chunk_start + chunk_size, len(target_indices))
            chunk_indices = target_indices[chunk_start:chunk_end]

            data_chunk = nf1.variables[value][start_idx + chunk_indices[0]:start_idx + chunk_indices[-1] + 1,
                                             loc[0]:loc[1], loc[2]:loc[3]]
        ...
        print(f"Warning: failed to process temperature file {filename}: {e}")
        raise ValueError(f"Unable to process temperature file: {filename}")

    if not monthly_temps:
        raise ValueError("No valid temperature data found")

    monthly_mean_2d = np.zeros((12, monthly_temps[1].shape[0], monthly_temps[1].shape[1]), dtype=np.float32)

    for month in range(1, 13):
        if monthly_counts[month] > 0:
            monthly_mean_2d[month-1, :, :] = monthly_temps[month] / monthly_counts[month]
        else:
            monthly_mean_2d[month-1, :, :] = np.nan

    coldest_month_2d = np.argmin(monthly_mean_2d, axis=0)

    days_in_month = np.array([31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31], dtype=int)
    cumulative_days = np.cumsum(np.concatenate([[0], days_in_month[:-1]]))
    coldest_day_2d = cumulative_days[coldest_month_2d] + 15

    coldest_day_indices = compressArray(coldest_day_2d, name="coldest_day", zeros=-1)

    print(f"Calculation complete. Coldest-month distribution: {np.bincount(coldest_month_2d.ravel())}")
    print(f"Compressed array shape: {coldest_day_indices.shape}")

    return coldest_day_indices
...
def writenetcdf(netfile,prename,addname,varunits,inputmap, timeStamp, posCnt, flag,flagTime, nrdays=None, dateunit="days",netcdfindex=False):
    ...
    if not flag:
        ...
        nf1.Source_Software = 'LAWS Python: ' + versioning['exe']
        ...
        nf1.source = 'LAWS output maps'
        nf1.acknowledgement = __acknowledgement__
...
def writeIniNetcdf(netfile,varlist, inputlist):
    ...
    nf1.Source_Software = 'LAWS Python'
    nf1.author = __author__
    nf1.institution = cbinding ("institution")
    nf1.title = cbinding ("title")
    nf1.source = 'LAWS initial-condition maps (based on CWatM)'
    nf1.acknowledgement = __acknowledgement__
    nf1.Conventions = 'CF-1.6'
