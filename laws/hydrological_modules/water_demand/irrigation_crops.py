...
from laws.management_modules.data_handling import returnBool, binding, cbinding, loadmap
import numpy as np
...
class waterdemand_irrigation_crops:
    """
    WATERDEMAND
    ...
    **Functions**
    ...
    def dynamic(self):
        ...
        * get non-Irrigation water demand and its return flow fraction
        ...
        potInf =  1 - np.maximum((1 - relSat), 0) ** self.var.arnoBeta[No]
        # ----------------------------------------------------------
        ...
        if No == 1 or No == 3:
            etpotMax = np.minimum(0.1 * (self.var.crop_weight_potT[No] + self.var.crop_weight_actSEV[No]) * 1000., 1.0)
        else:
        ...
        if "IrrigationThresholdFrac" in binding:
            IrrigationThreshold = loadmap("IrrigationThresholdFrac")
        else:
            IrrigationThreshold = 0.9

        self.var.pot_irrConsumption[No] = np.where(np.any(self.var.WS_Irr_daily < IrrigationThreshold, axis=0),
                                                    np.maximum(0.0, self.var.alphaDepletion * self.var.totAvlWater - readAvlWater),  0.)
