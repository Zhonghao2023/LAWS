...
class soil_crops(object):
    ...
    **Functions**
    ...
    # ------------ SOIL DEPTH ----------------------------------------------------------
        ...
        if 'GernalCropWaterDeficit' in binding:
            self.var.GernalCropWaterDeficit = loadmap('GernalCropWaterDeficit')
# --------------------------------------------------------------------------
# --------------------------------------------------------------------------
    ...
    # ---------------------------------------------------------
        ...
        if self.var.crop_coupling and No in (1, 3):

            ta1 = self.var.crop_weight_actT_soillayer[No][0]
            ta2 = self.var.crop_weight_actT_soillayer[No][1]
            ta3 = self.var.crop_weight_actT_soillayer[No][2]
            if np.isnan(ta1).any() or np.isnan(ta2).any():
                raise CWATMError("LAWS crop water uptake produced NaN values")
        else:
            etpotMax = np.minimum(0.1 * (self.var.cropCorrect * self.var.crop_correct_landCover[No] * self.var.cropKC[No] * self.var.ETRef * 1000.), 1.0)
        ...
        if (self.var.w1[No] < 0).any() or (self.var.w2[No] < 0).any() or (self.var.w3[No] < 0).any():
            raise CWATMError("LAWS soil-water update produced negative storage")
        ...
        if self.var.crop_coupling and No in (1, 3):
            self.var.actBareSoilEvap[No] = np.minimum(self.var.weight_actSEV_soillayer[No][0],np.maximum(0.,self.var.w1[No] - self.var.wres1[No])) \
                                            + np.minimum(self.var.weight_actSEV_soillayer[No][1],np.maximum(0.,self.var.w2[No] - self.var.wres2[No]))
            self.var.actBareSoilEvap[No] = np.where(self.var.FrostIndex > self.var.FrostIndexThreshold, 0., self.var.actBareSoilEvap[No])
            if np.any(self.var.actBareSoilEvap[No] < 0):
                raise CWATMError("LAWS bare-soil evaporation became negative")
        ...
        if self.var.crop_coupling and No in (1, 3):
            self.var.w1[No] = np.maximum(self.var.w1[No] - self.var.weight_actSEV_soillayer[No][0],self.var.wres1[No])
            self.var.w2[No] = np.maximum(self.var.w2[No] - self.var.weight_actSEV_soillayer[No][1],self.var.wres2[No])
        else:
            ...
            if No == 3:
                ...
                for c in range(len(self.var.Crops_names)):
                    ...
                    self.var.actTransTotal_crops_Irr[c] = np.where(
                        self.var.act_Transpt_Irr_daily[c]> 0,
                        self.var.act_Transpt_Irr_daily[c] * self.var.fracCrops_Irr[c],0)

                    self.var.ET_crop_Irr[c] = (self.var.actTransTotal_crops_Irr[c] +
                                               self.var.ES_with_Irrcrop[c] * self.var.fracCrops_Irr[c])

                    self.var.ET_crop_Irr_m[c] = np.where(self.var.fracCrops_Irr[c]>0,self.var.ET_crop_Irr[c] / self.var.fracCrops_Irr[c],0)
                    vars(self.var)['ET_crop_Irr_m'+str(c)] = self.var.ET_crop_Irr_m[c]

                    self.var.actTransTotal_month_Irr[c] += self.var.ET_crop_Irr[c]

                    self.var.actTransTotal_crops_nonIrr[c] = \
                        np.where(self.var.act_Transpt_nonIrr_daily[c] > 0,
                                 self.var.act_Transpt_nonIrr_daily[c] * self.var.fracCrops_nonIrr[c],0)

                    self.var.ET_crop_nonIrr[c] = (self.var.actTransTotal_crops_nonIrr[c] +
                                                  self.var.ES_with_nonIrrcrop[c] * self.var.fracCrops_nonIrr[c])

                    self.var.ET_crop_nonIrr_m[c] = np.where(self.var.fracCrops_nonIrr[c]>0,self.var.ET_crop_nonIrr[c] / self.var.fracCrops_nonIrr[c],0)
                    vars(self.var)['ET_crop_nonIrr_m'+str(c)] = self.var.ET_crop_nonIrr_m[c]

                    self.var.actTransTotal_month_nonIrr[c] += self.var.ET_crop_nonIrr[c]

                    self.var.irr_crop[c] = np.where(
                        self.var.frac_totalIrr * self.var.actTransTotal_crops_Irr[c]> 0, (
                                self.var.fracCrops_Irr[c] * self.var.actTransTotal_crops_Irr[c]) / self.var.weight_crop_actT[3],
