...
class initcondition(object):
    ...
    **Functions**
    ...
    def crops_initialise_cropmodule(self, xl_settings_file_path):
        pd = importlib.import_module("pandas", package=None)
        df = pd.read_excel(xl_settings_file_path)

        # Get all parameter names.
        parm = df.columns.tolist()

        # Store parameter values for all crops.
        for z in parm:
            # Convert the current parameter column to a list of crop values.
            other_crops_values = [val for idx, val in enumerate(df[z].tolist())]
            vars(self.var)[z] = other_crops_values
        ...
        if self.var.includeCrops:
            ...
            if 'Excel_settings_file' in binding:
                ...
                if self.var.crop_coupling:
                    self.crops_initialise_cropmodule(xl_settings_file_path)
                else:
