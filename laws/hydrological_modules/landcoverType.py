    ...
    def initial(self):
        ...
        * Water covered area No.5
        ...
        landcoverVars = ['irrTypeFracOverIrr','fractionArea','totAvlWater','cropKC', 'cropKC_landCover',
                         ...
                         'infiltration','actBareSoilEvap','landSurfaceRunoff','actTransTotal','weight_crop_actT',
                         'gwRecharge','interflow','actualET','pot_irrConsumption','act_irrConsumption','irrDemand',
                         'topWaterLayer',
                         'perc3toGW','capRiseFromGW','netPercUpper','netPerc','prefFlow','actBareSE_record']
        ...
        for coverType in self.var.coverTypes[:4]:
            ...
            # If the crop module is used,
            # the soil layers in the farmland will be divided to
            # take into account more precise water consumption by the crops.
            # Therefore, rain-fed farmland also needs to have its soil layers divided.
        ...
        for coverType in self.var.coverTypes[:4]:
            ...
            if coverType == 'grassland' or coverType == 'irrNonPaddy':
                self.var.w1[i] = (self.var.wfc1[i] - self.var.wwp1[i]) * 0.5 + self.var.wwp1[i]
                self.var.w2[i] = (self.var.wfc2[i] - self.var.wwp2[i]) * 0.5 + self.var.wwp2[i]
                self.var.w3[i] = (self.var.wfc3[i] - self.var.wwp3[i]) * 0.5 + self.var.wwp3[i]
        ...
        # for irrigation of non paddy -> No =3
        totalWaterPlant1 = np.maximum(0., self.var.wfc1[1] - self.var.wwp1[1]) #* self.var.rootDepth[0][3]
        totalWaterPlant2 = np.maximum(0., self.var.wfc2[1] - self.var.wwp2[1]) #* self.var.rootDepth[1][3]
        #totalWaterPlant3 = np.maximum(0., self.var.wfc3[3] - self.var.wwp3[3]) * self.var.rootDepth[2][3]
        self.var.totAvlWater_grassland = totalWaterPlant1 + totalWaterPlant2 #+ totalWaterPlant3

        self.var.Rain_times_fracPaddy = globals.inZero.copy()
        self.var.Rain_times_fracNonPaddy = globals.inZero.copy()
        if self.var.crop_coupling:
            ####Adding a new soil layer for a more detailed simulation of crop water absorption########################################################
            SPLIT_soilVars = ['SPLIT_w2','SPLIT_wfc2','SPLIT_wwp2','SPLIT_RZ','SPLIT_wres2','Rto1','Rto2']
            for variable in SPLIT_soilVars:  vars(self.var)[variable]= np.tile(globals.inZero,(self.var.soilLayers,4,1))
            SPLIT_soilVars2 = ['Rto1','Rto2']
            for variable in SPLIT_soilVars2:  vars(self.var)[variable]= np.tile(globals.inZero,(4,1))
            i = 0
            if "MinDepth_SPLIT_SOIL_LAYER" in binding:
                MinDepth_SPLIT_SOIL_LAYER = float(cbinding('MinDepth_SPLIT_SOIL_LAYER'))
                for coverType in self.var.coverTypes[:4]:
                    if coverType != 'forest' and coverType != 'irrPaddy':

                        ###The first soil layer in the second layer is fixed at 0.2 m
                        self.var.Rto1[i] = np.where(self.var.rootDepth[1][i] >= 0.2 +MinDepth_SPLIT_SOIL_LAYER,\
                                        (0.2-self.var.rootDepth[0][i])/self.var.rootDepth[1][i], 1)
                        self.var.SPLIT_w2[0][i] = self.var.w2[i] * self.var.Rto1[i]
                        self.var.SPLIT_wfc2[0][i] = self.var.wfc2[i] * self.var.Rto1[i]
                        self.var.SPLIT_wwp2[0][i] = self.var.wwp2[i] * self.var.Rto1[i]
                        self.var.SPLIT_RZ[0][i] = np.where(self.var.rootDepth[1][i] >= 0.2 +MinDepth_SPLIT_SOIL_LAYER,\
                                                    0.2-self.var.rootDepth[0][i],self.var.rootDepth[1][i])
                        self.var.SPLIT_wres2[0][i] = self.var.wres2[i] * self.var.Rto1[i]
                        ####If the remaining second-layer soil still meets the conditions for splitting, split it once again.
                        self.var.Rto2[i] = np.where(self.var.rootDepth[1][i]-self.var.SPLIT_RZ[0][i] >= 2 * MinDepth_SPLIT_SOIL_LAYER,\
                                        (self.var.rootDepth[1][i]-self.var.SPLIT_RZ[0][i]) / ( 2 * self.var.rootDepth[1][i]), 0)
                        self.var.SPLIT_w2[1][i] = self.var.w2[i] * self.var.Rto2[i]
                        self.var.SPLIT_wfc2[1][i] = self.var.wfc2[i] * self.var.Rto2[i]
                        self.var.SPLIT_wwp2[1][i] = self.var.wwp2[i] * self.var.Rto2[i]
                        self.var.SPLIT_RZ[1][i] = np.where(self.var.SPLIT_RZ[0][i] < self.var.rootDepth[1][i],\
                                                    (self.var.rootDepth[1][i]-self.var.SPLIT_RZ[0][i]) / 2,0)
                        self.var.SPLIT_wres2[1][i] = self.var.wres2[i] * self.var.Rto2[i]

                        self.var.SPLIT_w2[2][i] = self.var.w2[i] * self.var.Rto2[i]
                        self.var.SPLIT_wfc2[2][i] = self.var.wfc2[i] * self.var.Rto2[i]
                        self.var.SPLIT_wwp2[2][i] = self.var.wwp2[i] * self.var.Rto2[i]
                        self.var.SPLIT_RZ[2][i] = np.where(self.var.SPLIT_RZ[0][i] < self.var.rootDepth[1][i],\
                                                    (self.var.rootDepth[1][i]-self.var.SPLIT_RZ[0][i]) / 2,0)
                        self.var.SPLIT_wres2[2][i] = self.var.wres2[i] * self.var.Rto2[i]
                    i += 1
            elif self.var.crop_coupling:
                msg = "MinDepth_SPLIT_SOIL_LAYER not found in binding, Crop module will not work"
...
# --------------------------------------------------------------------------
    ...
    def dynamic(self):
        ...
        * calls sealed_water_module.dynamic
        ...
        # Crop water uptake is calculated once per model timestep. Calling it
        # inside the land-cover loop repeats cumulative crop calculations.
        if self.var.crop_coupling:
            self.model.crop_WU_module.dynamic()
